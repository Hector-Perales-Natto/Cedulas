...
<body onload="registrar.puesto.focus()">
	<div class="contenido">
		...
		<form name="registrar" class="form" method="POST" style="Width: 1000px;" >
			<table class="table">
				...
				<tr>
				<td class="form-group">
					<input type="text" class="form-control" required name="puesto" placeholder="Nombre del Puesto">
				</td>
				</tr>																						
				<tr>
				<td class="form-group">
...
if(isset($_POST['guardar'])) {
	$puesto = strtoupper(htmlspecialchars($_POST['puesto']));

	$sql = "SELECT * FROM puestos WHERE puesto = ?";
	$stmt = $conexion->prepare($sql);
	$stmt->execute(array($puesto));
	...
	if($checaPuesto) {
		...
		try {
			$sql = "INSERT INTO puestos (puesto, baja) values (?,?)";
			$stmt = $conexion->prepare($sql);
			$stmt->execute(array($puesto, 0));
			echo '<script type="text/javascript">
					swal({
						position: "top",
						text: "Ha agregado un puesto exitosamente!",
						confirmButtonColor: "#DD6B55"
					}).then(function() {
						window.location.href = "index.php?pagina=regPuesto.php";
					});
